...
"""
Train a UNCHA, HyCoCLIP, MERU or CLIP model based on parameters specified by a config file.
"""
import argparse
import time
import random
...
from uncha.config import LazyConfig, LazyFactory
...
def main(_A: argparse.Namespace):
    ...
    try:
        ...
        train_src = Path(__file__).resolve()
        model_src = (Path(__file__).parent.parent.parent / "uncha" / "models.py").resolve()

        shutil.copy2(train_src, backup_dir / "train.py")
        shutil.copy2(model_src, backup_dir / "models.py")

        logger.info(f"[Backup completed] Saved train.py and models.py to {backup_dir}")
    except Exception as e:
        logger.warning(f"[Backup failed] Could not copy train.py/models.py: {e}")
    ...
    if _A.resume and start_iteration > 0:
        print(f"!!!!!!!!!!!! Resumed training from iteration: {start_iteration} !!!!!!!!!!!!!")
    else:
        print(f"!!!!!!!!!!!! Starting fresh training from iteration: {start_iteration} !!!!!!!!!!!!!")
    ...
    for iteration in range(start_iteration + 1, _C.train.num_iterations + 1):
        ...
        with amp.autocast(enabled=_C.train.amp):
            if isinstance(getattr(model, "module", model), UNCHA):
        ...
        # Save intermediate checkpoint if needed.
