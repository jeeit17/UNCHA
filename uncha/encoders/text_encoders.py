...
class TransformerTextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.min_radius_head = nn.Sequential(
            nn.Linear(self.width, 1),
            nn.Softplus()  # ensures positive output
        )

    def get_min_radius(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, N, C), N includes cls + patches
        # cls_token = x[:, 0]  # (B, C)
        min_radius = self.min_radius_head(x)  # (B,)
        return min_radius
