...
from uncha.data.evaluation import CocoCaptions, Flickr30kCaptions
from uncha.evaluation.catalog import DatasetCatalog
from uncha.models import UNCHA, HyCoCLIP, MERU, CLIPBaseline
...
class ZeroShotRetrievalEvaluator:
    ...
    def __call__(self, model: UNCHA | HyCoCLIP | MERU | CLIPBaseline) -> dict[str, float]:
        model = model.eval()
        ...
        for dname in self._datasets:
            ...
            for _ids, _queries in zip(text_ids.split(256), text_feats.split(256)):
                ...
                if isinstance(model, (UNCHA, HyCoCLIP, MERU)):
            ...
            for _k in self._ks:
                results_dict[f"{dname}_image_r{_k}"] = _compute_recall(
                    text_to_image_retr, encoded_data["text_to_image_gt"], _k
                )

            for _k in self._ks:
                results_dict[f"{dname}_text_r{_k}"] = _compute_recall(
...
def _encode_dataset(
    ...
    model: UNCHA | HyCoCLIP | MERU | CLIPBaseline,
):
    """
    ...
    for inst in tqdm(data_loader, desc="Extracting image-text features", ncols=60):
