...
import uncha.data.evaluation as DE
...
class DatasetCatalog:
    ...
    CONSTRUCTORS: dict[str, Callable] = {
        ...
        "cub2011": DE.CUB2011,
        "sun397": partial(DE.TfdsWrapper, name="sun397"),
    ...
    def build(
        ...
        if name in ["aircraft", "country211", "imagenet", "sst2", "coco", "flickr30k", "cub2011"]:
            root = str(Path(root) / name)
        if name=="cars":
            root = "/path/to/your/cars/dataset"

        if name in ["mscoco", "voc"]:
            root = str(Path(root) / name)
